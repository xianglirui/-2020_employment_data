...
def changePage(key,city,p):
    ...
    for i in range(0,6):
        try:
            ...
            driver.refresh()
            changePage(key,city,p)



#爬取招聘单页的信息，并保存到mongodb
def manyPage(key,city):
    # 获得打开的第一个窗口句柄
...
if __name__ == '__main__':
    print("调用拉勾爬虫成功！")
    #sys.argv[1]
    click_City(sys.argv[1]+'站')
    job(sys.argv[2])
    p = page if sys.argv[3]==str(0) else int(sys.argv[3])
    try:
        changePage(sys.argv[2],sys.argv[1],p)
        print("success")
    except Exception:
        print("fail")
    print('执行完毕')
